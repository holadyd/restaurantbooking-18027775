...
def test_현재날짜가_일요일인_경우_예약불가_예외처리(mocker, customer):
    ...
    mock_get_now = mocker.patch(
        'booking_scheduler.BookingScheduler.get_now', return_value=datetime.strptime(SUNDAY, "%Y/%m/%d %H:%M")
    )
...
def test_현재날짜가_일요일이_아닌경우_예약가능(mocker, customer):
    # arrange
    mock_get_now = mocker.patch(
        'booking_scheduler.BookingScheduler.get_now', return_value=datetime.strptime(MONDAY, "%Y/%m/%d %H:%M")
    )
